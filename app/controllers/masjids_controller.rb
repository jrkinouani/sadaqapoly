class MasjidsController < ApplicationController
  ...
  # GET /masjids/new
  def new
    @masjid = current_user.masjids.build
  end
  ...
  def create
      @masjid = current_user.masjids.build(masjid_params)
  ...
  private
    ...
    def masjid_params
      params.require(:masjid).permit(:name, :city, :desc, :funding_goal, :amount_pledged, :end_date)
    end
end
